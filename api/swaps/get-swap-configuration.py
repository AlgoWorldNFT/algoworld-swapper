...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        dic = common.SwapConfigUrl(
            **{
                "ipfs_url": raw_params["ipfs_url"],
                "escrow": raw_params["escrow"],
                "chain": raw_params["chain"],
            }
        )
        ...
        gateway = (
            raw_params["gateway"] if "gateway" in raw_params else "ipfs.algonode.xyz"
        )
        config_file_url = f"https://{gateway}/ipfs/{dic.ipfs_url}/aw_swaps.json"
        ...
        try:
            ...
            configFile = json.dumps(
                [
                    ...
                    if "escrow" in config and config["escrow"] == dic.escrow
                ]
            )
