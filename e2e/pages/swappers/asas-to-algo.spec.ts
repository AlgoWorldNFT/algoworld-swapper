...
test(`should be able to perform ASAs to Algo swap`, async () => {
  await Promise.all([
    page.waitForNavigation(/*{ url: 'http://localhost:3000/swappers/asa-to-asa' }*/),
    page.locator(`button:has-text("ASAs to Algo")`).click(),
  ]);
  ...
  // Click text=Enter requesting Algo
  await page.locator(`text=Enter requesting Algo`).click();
  // Click [placeholder="Enter amount"]
  await page.locator(`[placeholder="Enter amount"]`).click();
  // Fill [placeholder="Enter amount"]
  await page.locator(`[placeholder="Enter amount"]`).fill(`1`);
  // Click div[role="dialog"] >> text=Select
  await page.locator(`div[role="dialog"] >> text=Select`).click();
